...
let update_player_in_list players updated_player =
  List.map
    (fun p ->
      if Player.get_id p = Player.get_id updated_player then updated_player
      else p)
    players
...
let create_game player_names initial_chips =
  ...
  let players =
    List.mapi
      (fun id name -> Player.create_player id name initial_chips)
      player_names
  in

  let deck = Card.create_deck () in

  { players; deck; pot = 0; community_cards = []; current_bet = 0 }
...
let handle_player_bet (gs : t) (player_id : int) (bet_amount : int) : t =
  ...
  let amount_actually_bet =
    ...
    let intended_total_for_player_this_round =
      current_player_bet_in_round + bet_amount
    in
    if
      intended_total_for_player_this_round
      >= chips_player_had + current_player_bet_in_round
    then chips_player_had
    else bet_amount
  in
...
let handle_player_raise (gs : t) (player_id : int)
    (new_total_bet_for_round : int) : t =
  let player = List.find (fun p -> Player.get_id p = player_id) gs.players in
  let player_current_contribution = Player.get_current_bet player in
  let chips_player_has = Player.get_chips player in

  let additional_chips_for_raise =
    new_total_bet_for_round - player_current_contribution
  in
...
let deal_initial_hands players deck =
  if players = [] then invalid_arg "No players to deal to"
    ...
    let rec deal acc_players cards_to_deal remaining_deck =
      ...
      | p :: ps ->
          ...
          let updated_player = { p with hand } in
...
let new_hand (gs : t) : t =
  ...
  let players_with_hands, remaining_deck =
    deal_initial_hands shuffled_state.players shuffled_state.deck
  in
  { shuffled_state with players = players_with_hands; deck = remaining_deck }

let reveal_community_cards (gs : t) (num_cards : int) : t =
  if num_cards <= 0 then gs
  else
...
let is_game_over g =
  let players_with_chips =
    List.filter (fun p -> Player.get_chips p > 0) g.players
  in
