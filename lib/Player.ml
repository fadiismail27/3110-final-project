...
let reset_for_new_hand player =
  { player with hand = []; folded = false; current_bet = 0; is_all_in = false }

let fold player = set_folded player true

(* let bet player amount = if amount < 0 then raise (Invalid_argument "Bet
   amount cannot be negative") else if amount > player.chips then { player with
   chips = 0; current_bet = player.current_bet + player.chips; is_all_in = true
   } else if amount = player.chips then { player with chips = 0; current_bet =
   player.current_bet + amount; is_all_in = true } else { player with chips =
   player.chips - amount; current_bet = player.current_bet + amount } *)

let bet player amount =
  if amount < 0 then raise (Invalid_argument "Bet amount cannot be negative")
...
let call player game_current_bet =
  ...
  if amount_to_call <= 0 then player
    ...
    {
      player with
      chips = player.chips - amount_to_call;
      current_bet = player.current_bet + amount_to_call;
    }
...
let raise_bet player new_total_bet =
  ...
  if amount_to_raise <= 0 then
    raise
      (Invalid_argument
         "Raise amount must be greater than current bet contribution")
    ...
    {
      player with
      chips = player.chips - amount_to_raise;
      current_bet = player.current_bet + amount_to_raise;
    }
