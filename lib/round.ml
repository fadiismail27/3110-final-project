...
let reset_player_bets_for_street (gs : Game.t) : Game.t =
  let updated_players =
    List.map (fun p -> Player.set_current_bet p 0) (Game.get_players gs)
  in
  { gs with players = updated_players; current_bet = 0 }

let get_starting_player_index (players : Player.t list)
    (dealer_button_pos : int) : int =
  ...
  if num_players = 0 then 0
    ...
    let rec find_next_active offset count =
      if count >= num_players then
        match
          List.find_index
            (fun p -> (not (Player.is_folded p)) && Player.get_chips p > 0)
            players
        with
        | Some i -> i
        | None -> 0
        ...
        if (not (Player.is_folded player)) && Player.get_chips player > 0 then
          p_idx
...
let handle_bot_action gs bot_player min_bet_val =
  ...
  let can_check =
    game_current_bet = 0 || bot_committed_bet = game_current_bet
  in
  ...
  let action =
    ...
    else if bot_chips <= amount_to_call && bot_chips > 0 then `AllIn
  ...
  | `Bet ->
      let bet_amount =
        max min_bet_val
          (min bot_chips (min_bet_val + Random.int ((bot_chips / 4) + 1)))
      in
      if bot_chips = 0 then (
        let next_gs = Game.handle_player_check gs (Player.get_id bot_player) in
        ...
        let next_gs =
          Game.handle_player_bet gs (Player.get_id bot_player) bet_amount
        in
  ...
  | `Raise ->
      let min_raise_total =
        game_current_bet + max min_bet_val (game_current_bet - bot_committed_bet)
      in
      let raise_amount =
        max min_raise_total
          (min
             (bot_chips + bot_committed_bet)
             (min_raise_total + Random.int ((bot_chips / 3) + 1)))
      in
      if
        bot_chips > amount_to_call
        && raise_amount > game_current_bet
        && raise_amount <= bot_chips + bot_committed_bet
      then (
        let next_gs =
          Game.handle_player_raise gs (Player.get_id bot_player) raise_amount
        in
      ...
      let next_gs, new_aggressor, was_aggressive =
        if game_current_bet = 0 || total_final_bet > game_current_bet then
          if game_current_bet = 0 then
            ( Game.handle_player_bet gs (Player.get_id bot_player)
                all_in_amount_as_bet,
              Some (Player.get_id bot_player),
              true )
          else
            ( Game.handle_player_raise gs (Player.get_id bot_player)
                total_final_bet,
              Some (Player.get_id bot_player),
              true )
        else (Game.handle_player_call gs (Player.get_id bot_player), None, false)
      in
...
let rec betting_round (gs : Game.t) (player_idx_to_act : int)
  ...
  let rec loop current_gs current_player_idx aggressor_id players_who_have_acted
      num_actions_this_bet_cycle =
    let current_players = Game.get_players current_gs in
    let player_to_act = List.nth current_players current_player_idx in

    let players_still_in_hand =
      List.filter (fun p -> not (Player.is_folded p)) current_players
    in
    if List.length players_still_in_hand <= 1 then current_gs
    else if
      num_actions_this_bet_cycle >= List.length players_still_in_hand
      && ((match aggressor_id with
          | None -> true
          | Some _ -> false)
         ||
         match aggressor_id with
         | Some id -> Player.get_id player_to_act = id
         | None -> false)
      && List.for_all
           (fun p ->
             Player.is_folded p
    ...
    then current_gs
    else if Player.is_folded player_to_act || Player.is_all_in player_to_act
    then
      loop current_gs
        ((current_player_idx + 1) mod num_total_players)
        aggressor_id players_who_have_acted
        (num_actions_this_bet_cycle + 1)
    else
      ...
      let new_gs, new_aggressor_id, was_aggressive_action =
        if Player.get_name player_to_act = "You" then begin
          ...
          | Check ->
              let next_gs =
                Game.handle_player_check current_gs
                  (Player.get_id player_to_act)
              in
              Printf.printf "%s checks.\n" (Player.get_name player_to_act);
              (next_gs, aggressor_id, false)
          | Bet amount ->
              let next_gs =
                Game.handle_player_bet current_gs
                  (Player.get_id player_to_act)
                  amount
              in
          ...
          | Call ->
              let next_gs =
                Game.handle_player_call current_gs (Player.get_id player_to_act)
              in
              Printf.printf "%s calls.\n" (Player.get_name player_to_act);
              (next_gs, aggressor_id, false)
          | Raise amount ->
              let next_gs =
                Game.handle_player_raise current_gs
                  (Player.get_id player_to_act)
                  amount
              in
          ...
          | Fold ->
              let next_gs =
                Game.handle_player_fold current_gs (Player.get_id player_to_act)
              in
          ...
          | AllIn ->
              ...
              let next_gs, resulting_aggressor_id, resulting_was_aggressive =
                if game_curr_bet_val = 0 || total_bet_value > game_curr_bet_val
                then
                  if game_curr_bet_val = 0 then
                    ( Game.handle_player_bet current_gs
                        (Player.get_id player_to_act)
                        chips,
                      Some (Player.get_id player_to_act),
                      true )
                  else
                    ( Game.handle_player_raise current_gs
                        (Player.get_id player_to_act)
                  ...
                  ( Game.handle_player_call current_gs
                      (Player.get_id player_to_act),
                    aggressor_id,
                    false )
              in
      ...
      let updated_players_acted =
        if List.mem (Player.get_id player_to_act) players_who_have_acted then
          players_who_have_acted
        else Player.get_id player_to_act :: players_who_have_acted
      in
      let new_num_actions =
        if was_aggressive_action then 1 else num_actions_this_bet_cycle + 1
      in
      let final_aggressor_id =
        if was_aggressive_action then new_aggressor_id else aggressor_id
      in
      loop new_gs next_player_idx final_aggressor_id updated_players_acted
        new_num_actions
  in
  let gs_street_ready =
    {
...
let play_hand_stages (state : Game.t) (dealer_button_pos : int)
  ...
  let starting_player_preflop =
    get_starting_player_index (Game.get_players hand_state) dealer_button_pos
  in
  (* Adjust for blinds *)
  let state_after_preflop_betting =
    betting_round hand_state starting_player_preflop min_bet_for_round
  in

  let active_players_preflop =
    List.filter
      (fun p -> not (Player.is_folded p))
      (Game.get_players state_after_preflop_betting)
  in
  if List.length active_players_preflop <= 1 then
    ...
    | [ winner ] ->
        ...
        let final_state =
          Game.distribute_pot state_after_preflop_betting
            [ Player.get_id winner ]
        in
        Printf.printf "\n--- End of Hand ---";
    ...
    let state_for_flop =
      reset_player_bets_for_street state_after_preflop_betting
    in
    ...
    let starting_player_flop =
      get_starting_player_index
        (Game.get_players state_after_flop_deal)
        dealer_button_pos
    in
    let state_after_flop_betting =
      betting_round state_after_flop_deal starting_player_flop min_bet_for_round
    in

    let active_players_flop =
      List.filter
        (fun p -> not (Player.is_folded p))
        (Game.get_players state_after_flop_betting)
    in
    if List.length active_players_flop <= 1 then
      ...
      | [ winner ] ->
          Printf.printf "\n🏆 Winner as only remaining player: %s\n"
            (Player.get_name winner);
          let final_state =
            Game.distribute_pot state_after_flop_betting
              [ Player.get_id winner ]
          in
          Printf.printf "\n--- End of Hand ---";
          Ui.print_game_state final_state dealer_button_pos;
      ...
      let state_for_turn =
        reset_player_bets_for_street state_after_flop_betting
      in
      let state_after_turn_deal =
        Game.reveal_community_cards state_for_turn 1
      in
      print_endline "\n-- Turn --";
      Ui.print_game_state state_after_turn_deal dealer_button_pos;
      let starting_player_turn =
        get_starting_player_index
          (Game.get_players state_after_turn_deal)
          dealer_button_pos
      in
      let state_after_turn_betting =
        betting_round state_after_turn_deal starting_player_turn
          min_bet_for_round
      in

      let active_players_turn =
        List.filter
          (fun p -> not (Player.is_folded p))
          (Game.get_players state_after_turn_betting)
      in
      if List.length active_players_turn <= 1 then
        ...
        | [ winner ] ->
            Printf.printf "\n🏆 Winner as only remaining player: %s\n"
              (Player.get_name winner);
            let final_state =
              Game.distribute_pot state_after_turn_betting
                [ Player.get_id winner ]
            in
            Printf.printf "\n--- End of Hand ---";
            Ui.print_game_state final_state dealer_button_pos;
        ...
        let state_for_river =
          reset_player_bets_for_street state_after_turn_betting
        in
        let state_after_river_deal =
          Game.reveal_community_cards state_for_river 1
        in
        print_endline "\n-- River --";
        Ui.print_game_state state_after_river_deal dealer_button_pos;
        let starting_player_river =
          get_starting_player_index
            (Game.get_players state_after_river_deal)
            dealer_button_pos
        in
        let state_after_river_betting =
          betting_round state_after_river_deal starting_player_river
            min_bet_for_round
        in

        let final_players = Game.get_players state_after_river_betting in
        let active_players =
          List.filter (fun p -> not (Player.is_folded p)) final_players
        in
        ...
        let winners_info =
          ...
          | _ ->
              ...
              print_endline "\n--- Showdown ---";
              let community_cards =
                Game.get_community_cards state_after_river_betting
              in
              let contenders =
                List.map
                  (fun p ->
                    ...
                    Printf.printf "%s shows: %s\n" (Player.get_name p)
                      (String.concat " "
                         (List.map Card.string_of_card (Player.get_hand p)));
              ...
              List.iter
                (fun (name, hand_rank) ->
                  Printf.printf "🏆 Winner: %s with %s\n" name
                    (Hand.string_of_hand_rank hand_rank))
                winning_hands_info;
              winning_hands_info
        in
        ...
        let winner_ids =
          List.map
            (fun (name, _) ->
              let player_obj =
                List.find (fun p -> Player.get_name p = name) final_players
              in
        ...
        let state_after_pot_distribution =
          ...
          else state_after_river_betting
        in
        Printf.printf "\n--- End of Hand ---";
